...
| EC2 host | `deploy/ec2/user-data.sh` writes `/etc/claude-code/managed-settings.json` | same script, `--as-user host-<name>` | Instance role → Secrets Manager |
...
| LLM egress | Only through LiteLLM → LiteLLM guardrails, logging and budgets apply to memory consolidation too (`x-litellm-tags: ai-memory`). |
...
- **Health**: `ai-memory status --json` (counts, spool, dropped events,
  provider health), LiteLLM spend by tag `ai-memory`.
